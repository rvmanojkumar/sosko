...
<div class="row">
            ...
            <form action="{{ route('admin.banners.update', $banner) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card-body">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="type" id="type" class="form-control @error('type') is-invalid @enderror" required>
                                    ...
                                    <option value="app_notification" {{ old('type', $banner->type) == 'app_notification' ? 'selected' : '' }}>App Notification</option>
                                </select>
                                @error('type')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="target_type" id="target_type" class="form-control">
                                    ...
                                    <option value="category" {{ old('target_type', $banner->target_type) == 'category' ? 'selected' : '' }}>Specific Category</option>
                                    <option value="vendor" {{ old('target_type', $banner->target_type) == 'vendor' ? 'selected' : '' }}>Specific Vendor</option>
                                </select>
                                @error('target_type')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="col-md-6" id="target_id_container" style="display: {{ old('target_type', $banner->target_type) ? 'block' : 'none' }};">
                            <div class="form-group">
                                <label for="target_id">Select Target</label>
                                <select name="target_id" id="target_id" class="form-control">
                                    <option value="">Select...</option>
                                    @if(old('target_type', $banner->target_type) == 'category')
                                        @foreach($categories ?? [] as $category)
                                            <option value="{{ $category->id }}" {{ old('target_id', $banner->target_id) == $category->id ? 'selected' : '' }}>
                                                {{ $category->name }}
                                            </option>
                                        @endforeach
                                    @elseif(old('target_type', $banner->target_type) == 'vendor')
                                        @foreach($vendors ?? [] as $vendor)
                                            <option value="{{ $vendor->id }}" {{ old('target_id', $banner->target_id) == $vendor->id ? 'selected' : '' }}>
                                                {{ $vendor->store_name }}
                                            </option>
                                        @endforeach
                                    @endif
                    ...
                    <div class="form-group">
                        ...
                        @error('image')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                        <small class="text-muted">Recommended size: 1200x400px. Max 5MB. Leave empty to keep current image.</small>
                    </div>
    ...
    <div class="col-md-4">
            ...
            <div class="card-body">
                <div id="preview-container" class="text-center">
                    ...
                    <div class="text-left mt-3">
                        ...
                        <p id="preview-subtitle" class="text-muted mb-2">{{ $banner->subtitle }}</p>
                        <span id="preview-cta" class="badge badge-primary">{{ $banner->cta_text ?: 'No CTA' }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <p><strong>Created:</strong> {{ $banner->created_at->format('d M Y, h:i A') }}</p>
                <p><strong>Last Updated:</strong> {{ $banner->updated_at->format('d M Y, h:i A') }}</p>
...
<script>
    ...
    function removeImage(bannerId) {
        if (confirm('Are you sure you want to remove this image?')) {
            fetch(`/admin/banners/${bannerId}/remove-image`, {
                method: 'DELETE',
                headers: {
